...
## 1. Research & Design
- [ ] **Analyze KiCad File Formats:** Deep dive into format specifications for `.kicad_sym` (symbols) and `.kicad_mod` (footprints).
- [ ] **Map Primitives:** Determine how `acadlisp` primitives (`LINE`, `CIRCLE`, `ARC`, `TEXT`) map to KiCad's graphical elements.
- [ ] **Define Lisp Extensions:** Design new Lisp functions needed for specific KiCad entities not covered by generic CAD commands (e.g., pins, pads, electrical properties).

## 2. Symbol Export (`.kicad_sym`)
- [ ] **Implement `kicad-pin` Function:** Add a Lisp command to define symbol pins (number, name, electrical type, position).
- [ ] **Implement `kicad-property` Function:** Support adding metadata fields (Reference, Value, Footprint, Datasheet).
- [ ] **Create Symbol Exporter:** Implement a Rust module to traverse the `DrawEntity` list and serialize it into the `.kicad_sym` S-expression format.
- [ ] **Verify Symbol Geometry:** Ensure lines, circles, and arcs are correctly transformed and scaled for schematic grids.
...
- [ ] **Create Test Suite:** Develop a set of standard Lisp scripts that generate known KiCad components (e.g., "555 Timer Symbol", "SOIC-8 Footprint").
